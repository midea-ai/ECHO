...
scorer = SEMBScoreMetric(model_path=model_path)
...
def _func(item):
    ...
    try:
        ...
        finding, impression = baichuan_llm.extract_info(response)
        response = f"Findings: {finding} Impression: {impression}"
    ...
    try:
        ...
        finding, impression = baichuan_llm.extract_info(gt)
        gt = f"Findings: {finding} Impression: {impression}"
    except Exception as e:
        print(e)
        return False, None

    return True, {"response": response, "golden": gt}
    ...
    with ThreadPool(THREAD) as pool:
        with tqdm(total=len(data), desc="Processing Rows") as pbar:
            for result in pool.imap_unordered(_func, data):
                flag, item = result
                if flag:
                    reports.append(item["golden"])
                    preds.append(item["response"])
                else:
                    fail_nums += 1
                pbar.update(1)

    results = scorer.compute_rewards(preds, reports)
    avg = np.mean(np.array(results))
    print(f"SEMBScoreMetric results: {avg:.4f}")
    print(f"Fail Rate: {fail_nums}/{len(data)}")
