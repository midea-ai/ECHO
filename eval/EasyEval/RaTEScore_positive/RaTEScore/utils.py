...
def post_process(tokenized_text, predicted_entities, tokenizer):
    ...
    for start, end, entity_type in entity_spans:
        ...
        # print(f"entity: {entity_str}, type: {entity_type}")
...
def compute(gt_embeds_word, pred_embeds_word, gt_types, pred_types, weight_matrix):
    # gt_embeds_word.shape = [sub_sent_list_len, dim]
    # pred_embeds_word.shape = [sub_sent_list_len, dim]
    # len(gt_types) = sub_sent_list_len
    # len(pred_types) = sub_sent_list_len
    ...
    neg_weight = weight_matrix[("NEG", "WEIGHT")]  # float
    # topk_indices: per-gt index of nearest pred
    ...
    type_score = [neg_weight if (gt_type, pred_type) in neg_class else 1 for gt_type, pred_type in
                  zip(gt_types, topk_map)]
