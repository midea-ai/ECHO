"""
Profile LLaVA-Med for tokens/s throughput.
Uses LlavaForConditionalGeneration + AutoProcessor with chat_template from chat_template.json.
"""
import argparse
import time
import torch
...
def profile_autoregressive(model, inputs, processor, args):
    """LLaVA-Med: pixel_values, input_ids, attention_mask."""
    model.eval()
    prof = Profiler()
...
def profile_simple_generate(model, inputs, processor, args):
    """Use model.generate() for throughput measurement."""
    model.eval()
    prof = Profiler()
    ...
    prof.start()
    out = model.generate(**inputs, max_new_tokens=args.gen_length)
    prof.record("total")
...
def main():
    ...
    parser.add_argument("--model_dir", type=str, default="microsoft/llava-med-v1.5-mistral-7b")
    parser.add_argument("--input_json", type=str, default="minic_100.json")
    parser.add_argument("--out_dir", type=str, default="./profile_results_llava_med")
    ...
    parser.add_argument("--simple", action="store_true", help="Use model.generate() for throughput (no breakdown)")
    ...
    for i, sample in enumerate(samples):
        try:
            ...
            # LLaVA-med message format
            content = [{"type": "image"}] * len(imgs) + [{"type": "text", "text": prompt_text}]
            ...
            inputs = processor(images=imgs, text=prompt, return_tensors="pt").to(model.device, torch.bfloat16)
    ...
    json_path = os.path.join(args.out_dir, f"profile_results_llava_med_{int(time.time())}.json")
    ...
    # Report
    print("\n" + "=" * 70)
    print(f"LLaVA-Med INFERENCE BREAKDOWN (Total Tokens: {total_tokens_generated})")
    print("=" * 70)
